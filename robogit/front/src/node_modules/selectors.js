...
export const getInformations = (state, ownProps) => {
    ...
    var filterActive = state.informationsPage.filterActive;
    var filterIds = state.informationsPage.filterIds;
    console.dir(filterActive);
    console.dir(filterIds);
    ...
    for (let i = 0; i < state.informationsPage.ids.length; i++) {
        ...
        if (applySearch(info) && (!filterActive || (filterActive && filterIds.includes(id)))) {
            switch (activeCategoryId) {
                case '1':
                    info.type === 'CONTROLLER' ? res.push(info) : null;
                    break;
                case '2':
                    info.type === 'PLATFORM' ? res.push(info) : null;
                    break;
                case '3':
                ...
                case '5':
                    ...
                    break;
                case '6':
                    info.type === 'OTHER_RESOURCES' ? res.push(info) : null;
                    break;
                default:
                    res.push(info)
            }
